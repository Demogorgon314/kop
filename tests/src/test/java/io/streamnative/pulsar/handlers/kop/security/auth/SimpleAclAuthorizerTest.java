...
public class SimpleAclAuthorizerTest extends KopProtocolHandlerTestBase {
    ...
    @Test
    public void testAuthorize() throws ExecutionException, InterruptedException {
        Boolean isAuthorized = simpleAclAuthorizer.authorize(
                new KafkaPrincipal(KafkaPrincipal.USER_TYPE, USER_NAME + "/" + SIMPLE_USER),
                AuthAction.produce,
                Resource.of(ResourceType.TOPIC, TOPIC)).get();
        assertTrue(isAuthorized);

        isAuthorized = simpleAclAuthorizer.authorize(
                new KafkaPrincipal(KafkaPrincipal.USER_TYPE, USER_NAME + "/" + ANOTHER_USER),
                AuthAction.produce,
                Resource.of(ResourceType.TOPIC, TOPIC)).get();
